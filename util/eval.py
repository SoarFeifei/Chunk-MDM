...
def compute_ground_pen(foot_idx, positions, thres):
    ...
    below_thres = contact_zs[contact_zs < thres]
    if below_thres.size > 0:
        contact_zs_mean = below_thres.mean()
    else:
        contact_zs_mean = 0
        # contact_event = np.sum(contact_zs < 0)/(positions.shape[0])   ###
    has_penetration = np.any(contact_zs < thres, axis=1)  # 形状: (seq_len,)
    contact_event = np.mean(has_penetration)
    # print("contact_event", contact_event)
    # print("np.sum(contact_zs < 0)", np.sum(contact_zs < 0), positions.shape[0])
    # print("contact_zs", contact_zs.max(), contact_zs.min())
    return contact_zs_mean, contact_event

###Acceleration
def compute_Acc(positions):
    """
        positions: (B, T, J, 3) or (T, J, 3)
    """
    ...
    accel_norm = np.linalg.norm(accel, axis=-1)  # (B, T-2, J)
...
def compute_ade(output_seqs, ref_clip):
    ...
    for i,seq in enumerate(output_seqs):
        # print("seq", seq.shape, ref_clip.shape)
        dist_joint_frame = np.linalg.norm(seq-ref_clip,axis=-1)
        # print("dist_joint_frame", dist_joint_frame.shape)
        # print("dist_joint_frame", dist_joint_frame.shape)
    ...
    return dist_min, dist_min_last, min_idx
