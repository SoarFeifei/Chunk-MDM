...
class BaseMotionData(data.Dataset):
    ...
    def __init__(self, config):
        ...
        if self.load_full_data:
            if osp.exists(osp.join(self.path, 'data.npz')) and self.load_cache:
                ...
                data_reshape = self.motion_flattened.reshape(int(num_block * self.block_size), int(frame_dim))
                self.motion_flattened = data_reshape  # num_block * block_size，frame_dim(single_dim)
    ...
    def get_heading_dr(self, data):
        if self.data_root_rot_dim > 1:
            ...
            # print("data",data.shape)    # torch.Size([512, 267])
            # print("data_root_linear_dim", self.data_root_linear_dim)    # 2
