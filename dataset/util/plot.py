...
def plot_jnt_vel(jnts_multiple, links_single, plot_fn, fps, save_path):
    # if len(jnts_multiple.shape) == 3:
    #     jnts_multiple = jnts_multiple[None, ...]
    # print("jnts_multiple.shape", len(jnts_multiple.shape))
    assert len(jnts_multiple.shape) == 4, 'wrong dimension for multple char plotting'
    num_char, num_frame, num_jnt, num_dim = jnts_multiple.shape
    ...
    links = np.concatenate([np.array(links_single) + j*num_jnt for j in range(num_char-3)],axis=0)  ### num_char-1
