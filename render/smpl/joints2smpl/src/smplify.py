...
# from render.smpl.joints2smpl.src import config
...
class SMPLify3D():
    ...
    def __call__(self, init_pose, init_betas, init_cam_t, j3d, conf_3d=1.0, seq_ind=0):
       ...
       # -------------Step 1: Optimize camera translation and body orientation--------
            ...
            for i in range(10): ### 10
                def closure():
                    ...
                    with torch.no_grad():
                        ...
                        body_pose[:, 33:36] = 0.0   # neck
                        body_pose[:, 42:45] = 0.0   # head
                        # # body_pose[:, 45:48] = 0.0   # Lshoulder
                        # # body_pose[:, 48:51] = 0.0   # Rshoulder
                        # body_pose[:, 36:39] = 0.0  # LCollar
                        # body_pose[:, 39:42] = 0.0  # RCollar
                    smpl_output = self.smpl(global_orient=global_orient,
                                            body_pose=body_pose,
                                            betas=betas)
                    model_joints = smpl_output.joints
        ...
        ###
        # with torch.no_grad():
            # 这里赋予一个向外张开的旋转角度 (Axis-Angle 格式)
            # 具体的轴向(X,Y,Z)取决于你的坐标系，通常绕 Z 轴或 Y 轴旋转可以撑开肩膀。
            # 这里的 [0, 0, 0.2] 是左锁骨，[0, 0, -0.2] 是右锁骨，你可以根据预览效果微调数值
            # body_pose[:, 36:39] = torch.tensor([0.0, 0.0, 0.15]).to(self.device)
            # body_pose[:, 39:42] = torch.tensor([0.0, 0.0, -0.15]).to(self.device)
